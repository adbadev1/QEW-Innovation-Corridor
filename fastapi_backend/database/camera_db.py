...
class CameraDatabase:
    ...
    def insert_image(self, image_data: Dict) -> int:
        ...
        # Convert filename to relative path if it's absolute
        filename = image_data.get('filename')
        if filename:
            filename = self._to_relative_path(filename)
    ...
    def create_collection(self, collection_id: str, output_directory: str) -> str:
        """Create a new collection record with relative path"""
        # Convert output_directory to relative path
        rel_output_dir = self._to_relative_path(output_directory)

        self.cursor.execute('''
            INSERT INTO collections
            (collection_id, start_time, output_directory, status)
            VALUES (?, ?, ?, ?)
        ''', (collection_id, datetime.now(), rel_output_dir, 'in_progress'))
        self.conn.commit()
    ...
    def get_images_by_collection(self, collection_id: str) -> List[Dict]:
        """Get all images for a collection with absolute paths"""
        ...
        return [self._convert_paths_to_absolute(img, ['filename']) for img in images]

    def get_images_by_camera(self, camera_id: int) -> List[Dict]:
        """Get all images for a camera with absolute paths"""
    ...
    def get_collection_stats(self, collection_id: str) -> Optional[Dict]:
        """Get statistics for a collection with absolute paths"""
        self.cursor.execute('SELECT * FROM collections WHERE collection_id = ?', (collection_id,))
        row = self.cursor.fetchone()
        if row:
            collection = dict(row)
            return self._convert_paths_to_absolute(collection, ['output_directory'])
        return None

    def get_all_collections(self) -> List[Dict]:
        """Get all collections with absolute paths"""
        self.cursor.execute('SELECT * FROM collections ORDER BY start_time DESC')
        collections = [dict(row) for row in self.cursor.fetchall()]
        # Convert relative paths to absolute
        return [self._convert_paths_to_absolute(col, ['output_directory']) for col in collections]

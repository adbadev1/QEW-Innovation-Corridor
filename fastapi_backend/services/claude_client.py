...
confidence: [0.0-1.0]
lanes_detected: [Which lanes visible? e.g. Northbound, Westbound]
lane_counts: [Count per direction]
road_features: [Ramps, bridges, etc.]
...
- For lanes, determine if the camera sees the main highway lanes and which direction they travel
    ...
    def parse_toon_response(self, response: str) -> Dict:
        ...
        for line in lines:
            if ':' in line:
                ...
                elif key == 'lanes_detected':
                    result['lanes_detected'] = value
                elif key == 'lane_counts':
                    result['lane_counts'] = value
                elif key == 'road_features':
                    result['road_features'] = value
                elif key == 'landmarks':

...
class GeminiDirectionAnalyzer:
    ...
    def analyze_direction(self, satellite_image_path: str, camera_image_path: str,
        ...
        Args:
            ...
            camera_info: Optional dictionary with camera metadata (location, coordinates, etc.)
        ...
        # Prepare prompt (can use camera_info if needed)
...
CONFIDENCE: [0.0-1.0]
LANES_DETECTED: [Which lanes are visible? e.g., "Northbound", "Westbound", "Both", "Ramp"]
LANE_COUNTS: [Number of lanes visible per direction, e.g., "3 Northbound, 3 Southbound"]
ROAD_FEATURES: [Comma-separated list of features: e.g., "Collector Lanes", "On-Ramp", "Off-Ramp", "Bridge", "Overpass", "Side Road"]
LANDMARKS: [Brief description of visible landmarks]
REASONING: [Brief explanation of how you determined the direction]

Be precise and use the 16-point compass system. For lanes, determine if the camera sees the main highway lanes and which direction they travel."""
        ...
        for line in lines:
            ...
            if line.startswith('DIRECTION:'):
                ...
                try:
                    ...
                    pass
            elif line.startswith('LANES_DETECTED:'):
                result['lanes_detected'] = line.split(':', 1)[1].strip()
            elif line.startswith('LANE_COUNTS:'):
                result['lane_counts'] = line.split(':', 1)[1].strip()
            elif line.startswith('ROAD_FEATURES:'):
                result['road_features'] = line.split(':', 1)[1].strip()

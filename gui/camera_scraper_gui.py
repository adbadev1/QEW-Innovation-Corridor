"""
QEW Camera Collection GUI - Integrated with FastAPI Backend
...
class CameraCollectionWorker(QThread):
    """Worker thread for camera image collection with detailed logging"""
    progress = pyqtSignal(str)
    finished = pyqtSignal(bool, str)

    def __init__(self, backend_manager: BackendManager, images_per_camera=1, delay_between_captures=60):
        super().__init__()
        self.backend_manager = backend_manager
        self.images_per_camera = images_per_camera
        self.delay_between_captures = delay_between_captures
        self.is_running = True
        self.scraper = None

    def run(self):
        """Run the camera collection process with detailed logging"""
        try:
            # Import the scraper service
            from fastapi_backend.services.camera_scraper import CameraScraperService

            # Create scraper with progress callback
            self.scraper = CameraScraperService()
            self.scraper.progress_callback = self.progress.emit

            self.progress.emit("=" * 80)
            self.progress.emit("QEW Corridor Camera Image Collection")
            self.progress.emit("=" * 80)

            # Load camera data
            self.progress.emit("Loading camera data...")
            cameras = self.scraper.load_camera_data()
            self.progress.emit(f"✓ Loaded {len(cameras)} cameras from QEW corridor")

            # Calculate estimates
            total_views = sum(len(c.get('Views', [])) for c in cameras)
            estimated_images = total_views * self.images_per_camera
            self.progress.emit(f"Estimated images to collect: {estimated_images}")

            if self.images_per_camera > 1:
                estimated_time = (self.images_per_camera - 1) * self.delay_between_captures / 60
                self.progress.emit(f"Estimated time: {estimated_time:.1f} minutes")

            self.progress.emit("")

            # Create output directory
            output_dir = self.scraper.create_output_directory()

            # Run the scraping
            result = self.scraper.scrape_cameras(
            ...
            if result['status'] == 'success':
                self.progress.emit("")
                self.progress.emit("=" * 80)
                self.progress.emit("All done! Images ready for AI hazard assessment.")
                self.progress.emit("=" * 80)
                self.finished.emit(True, "Collection completed successfully")
            else:
                error_msg = result.get('error', 'Unknown error')
                self.progress.emit(f"✗ Collection failed: {error_msg}")
                self.finished.emit(False, f"Collection failed: {error_msg}")
    ...
    def stop(self):
        ...
        if self.scraper:
            ...
            self.progress.emit("Stopping collection...")
...
class QEWCameraGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        # Start backend and load camera data
        self.startup_backend()
    
    def startup_backend(self):
        """Start backend and load initial data"""
        # Start backend
    ...
    def load_camera_metadata(self):
        """Load camera metadata from backend API"""
        try:
            url = self.backend_manager.get_api_url('/api/stats')
            response = requests.get(url, timeout=5)

            if response.status_code == 200:
                stats = response.json()
                self.camera_count = stats.get('total_cameras', 0)
                self.log(f"Loaded {self.camera_count} cameras from backend")
            else:
                self.log("Warning: Could not load camera stats from backend")
                self.camera_count = 0
        except Exception as e:
            self.log(f"Error loading camera data: {e}")
            self.camera_count = 0
    ...
    def create_control_section(self):
        ...
        self.manual_button = QPushButton("MANUAL COLLECTION")
    ...
    def create_status_section(self):
        ...
        group = QGroupBox("Status & Activity Log")
        group.setStyleSheet("""
            QGroupBox {
                font-size: 14px;
                font-weight: bold;
    ...
    def log(self, message):
        ...
        # Update camera count label if we have the count
        if hasattr(self, 'camera_count'):
            self.camera_count_label.setText(f"Cameras: {self.camera_count}")
    ...
    def on_timezone_changed(self, timezone):
        ...
        self.save_settings()
        self.update_clock()
        self.log(f"Timezone changed to {timezone}")
    ...
    def start_collection(self):
        ...
        if interval_hours == 0 and interval_minutes == 0:
            self.log("Error: Please set a collection interval greater than 0")
            return
        ...
        self.log(f"Automatic collection started (every {interval_hours}h {interval_minutes}m)")
        self.update_next_collection_time()
    ...
    def stop_collection(self):
        ...
        self.log("Automatic collection stopped")

    def manual_collection(self):
        """Run a manual collection"""
        if self.worker and self.worker.isRunning():
            self.log("Collection already in progress...")
            return
    ...
    def run_collection(self):
        ...
        if self.worker and self.worker.isRunning():
            self.log("Previous collection still in progress, skipping...")
            return

        self.log("Starting camera image collection...")
        ...
        delay_between_captures = 60  # Default delay

        self.worker = CameraCollectionWorker(
